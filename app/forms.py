...
class CreateBlogForm(forms.ModelForm):
    ...
    category = forms.ModelChoiceField(queryset=Category.objects.all(), widget=forms.Select(attrs={'class': 'form-control','style':"font-size:14px; padding:10px; height:40px"}))

    class Meta:
        model = Blog
        fields = ['title','short_description','image','long_description','is_featured','tags','category']